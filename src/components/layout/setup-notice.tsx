...
export function SetupNotice() {
  ...
  return (
    <div role="status" className="border-butter-deep/50 bg-butter/60 text-chocolate border-b px-4 py-2 text-center text-sm">
      ...
      {problems.map((p, i) => (
        <span key={p.name}>
          {i > 0 ? ", " : ""}
          <code className="bg-paper rounded px-1">{p.name}</code> ({p.reason})
        </span>
      ))}
      .
    </div>
