...
### 1000 Timeline (the product)

| Table                     | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `regions`                 | The cosmos, the whole Earth, and eleven world regions with lat/lng for Atlas pins.                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `timeline_entries`        | Everything on the timeline. `kind` (era, event, organism, civilization, person, invention, artwork, discovery, place, extinction), `start_year`/`end_year` (negative = BCE as written; deep time as large negatives), `is_ongoing`, `parent_slug` + `level` for nested eras, `region_slug` + `lat`/`lng`, `importance` (3 shows at any zoom), optional `min_span_years` gate, `age_band` (1: 5–7 … 4: older), `curiosity_key` (which Mibbi notices it), and the four lines a child reads: `tagline`, `what`, `where_text`, `why`. |
| `entry_relations`         | led_to / caused / overlapped / part_of / inspired / ended between entries.                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `adventures`              | Story expeditions: companion, optional `unlock_character_slug` (a physical Mibbi), `start_entry_slug`, and `steps` JSON `[{title, text, entry_slug, companion_line}]`.                                                                                                                                                                                                                                                                                                                                                            |
| `user_discoveries`        | The museum: one row per entry a child has kept, with how it was found.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `user_adventure_progress` | Step index and completion per adventure.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `concept_signals`         | Invisible assessment: one row per judgement (ordering, scale, simultaneity…), never shown as a grade.                                                                                                                                                                                                                                                                                                                                                                                                                             |

Function `entries_around_year(year, tolerance?)` returns non-era entries active around a year; tolerance defaults to 5% of years-ago (min 50), so deep-time moments still find neighbours. It powers the Atlas and "What else was happening?".

`characters.curiosity_key` (fossil | sprout | wonder | story | maker | muse | explorer) was added here so companions can notice matching entries.
...
- **Timeline content** (`regions`, `timeline_entries`, `entry_relations`, `adventures`): public read, admin write. **Discoveries, progress, signals**: the child's own rows; plain inserts under RLS because nothing here moves value.
